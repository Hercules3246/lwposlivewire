...
class DashController extends Component
{
    ...
    {
        $dt = new DateTime(); //2022-06-01 3:34:20:40.8882
        $startDate = null;
        $finishDate = null;

        for ($d=1; $d <= 7; $d++) {
            //norma ISO 8601 // year/month/date
    ...
    public function getSalesMonth()
    {
        $salesByMonth = DB::select(
            ...
            FROM (SELECT 'january' AS month UNION SELECT 'february' AS month  UNION SELECT 'march' AS month  UNION SELECT 'may' AS month  UNION SELECT 'june' AS month  UNION SELECT 'july' AS month  UNION SELECT 'august' AS month  UNION SELECT 'september' AS month  UNION SELECT 'october' AS month  UNION SELECT 'november' AS month  UNION SELECT 'december' AS month ) m LEFT JOIN (SELECT MONTHNAME(created_at) AS MONTH, COUNT(*) AS orders, SUM(total) AS total
